...
class KegiatanController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope)) && !Auth::user()->can('read_all', app($dataType->model_name))) {
    ...
    public function show(Request $request, $id)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            } else if (Auth::user()->can('read_all', app($dataType->model_name))) {
                $model = $model::select('*');
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        // add user id
        $request->merge([
            'id_user' =>  $request->kegiatan_belongsto_user_relationship,
        ]);

        //add kontak
        $request->merge([
            'id_kontak' =>  $request->kegiatan_belongsto_kontak_relationship,
        ]);
    ...
    {
        $id         = $request->get('id');
        $results    = DB::table('kabkots')->where('provinsi_id', $id)->get();
    ...
    {
        $id         = $request->get('id');
        $results    = DB::table('kecamatans')->where('kabkot_id', $id)->get();
        return response()->json($results);
    }
    ...
    {
        $id         = $request->get('id');
        $results    = DB::table('kelurahans')->where('kecamatan_id', $id)->get();
        return response()->json($results);
    }
    ...
    {
        $id         = $request->get('id');
        $results    = DB::table('kontaks')->where('id_user', $id)->get();
        return response()->json($results);
    }
}